...
if subprocess.run(["emacsclient", "--server-file", server_name, "--eval", "(+ 2 2)"]).returncode != 0:
    # 如果Emacs server未运行，则启动之
    start_emacs_server()

try:
    # 使用 emacsclient 打开文件
    # subprocess.Popen(["emacsclient","-s" ,"--server-file", server_name, "-c", "-n", file_path, line_num])
    subprocess.Popen(["emacsclient" ,"--server-file", server_name, "-n", file_path, line_num])
