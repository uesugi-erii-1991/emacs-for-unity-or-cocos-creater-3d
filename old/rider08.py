...
try:
    ...
    logging.error("获取命令行参数失败！错误信息：%s" % str(e))
    sys.exit(0)

# 检测 Emacs server 是否正在运行
if subprocess.run(["emacsclient", "-s", server_name, "--eval", "(+ 2 2)"]).returncode != 0:
    # 如果Emacs server未运行，则启动之
    start_emacs_server(server_name)

try:
    # 使用 emacsclient 打开文件
    subprocess.Popen(["emacsclient" ,"-s", server_name, "-n", file_path, line_num])
    logging.info("成功打开文件 %s ，指定行号为：%s" % (file_path, line_num))
except Exception as e:
    logging.error("打开文件失败！错误信息：%s" % str(e))
    sys.exit(0)
input(00)